...
server <- function(input, output, session) {
  ...
  resolutionchoices_rv <- reactiveValues(
    ...
    selected_set = list(
      NDVItsTab = "Sentinel_1000",
      NDVIdeltaTab = "Sentinel_1000",
      LCexplorerTab = "Sentinel_1000",
      BAexplorerTab = "500"
    )
  )
  ...
  output$ndvi_ts_plot_container <- renderUI({
    ...
    imageOutput("ndvi_ts_plot_output", width = "100%", height = "auto")
  })
  ...
  output$lc_plot_container <- renderUI({
    # Default UI is just an image placeholder
    fluidRow(
      column(7, imageOutput("ndvi_plot_output", height = "auto")),
  ...
  output$dm_plot_container <- renderUI({
    # Default UI is just an image placeholder
    fluidRow(
      column(6, imageOutput("ndvi_histmap_output", height = "auto")),
      column(6, htmlOutput("ndvi_delta_map_output"))
    )
  })  
  ...
  observeEvent(input$generate_ndvi_delta_plot, {
    ...
    tryCatch({
      ...
      output$ndvi_delta_map_output <- renderUI({
        tags$iframe(src = paste0("figures/", figure_filename_dm), width = "100%", height = "500px", frameborder = 0)
      })
    }, error = function(e) {
      output$ndvi_delta_map_output <- renderUI({
  ...
  # ---------------------------------------------------------------------------------------------------
  # BURNED AREA MAP AND FUNCTIONALITY
  # ---------------------------------------------------------------------------------------------------  
  ...
  output$ba_plot_container <- renderUI({
    # Default UI is just an image placeholder
    fluidRow(
      column(6, imageOutput("ba_plot_output", height = "auto")),
      column(6, imageOutput("ba_map_output", height = "auto"))
    )
  })  
  ...
  # Observe the Generate Figure button
  observeEvent(input$generate_ba_figures, {
    ...
    tryCatch({
      ...
      output$ba_map_output <- renderImage({
        list(src = figure_path_bam, width = "100%", alt = "Burned Area 2D map")
      }, deleteFile = FALSE)
